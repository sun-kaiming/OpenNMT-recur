...
def check_hypothesis(eq, n_predictions=5):
    """
    ...
    eq["hyp"] = hyp
    tree = eq["tree"]
    error = env.check_prediction(src, tgt, hyp, tree, n_predictions)
    eq["error"] = error
...
class Evaluator(object):
    ...
    def enc_dec_step_beam(self, data_type, task, scores):
        ...
        decoder
        print("multi_gpu ",params.multi_gpu)
        encoder.eval()
        decoder.eval()
        ...
        for (x1, len1), (x2, len2), trees, infos in iterator:
            ...
            # cuda
            x1_, len1_, x2, len2, y = to_cuda(x1, len1, x2, len2, y, use_cpu=params.cpu)
            bs=len1_.shape[0]
            n_total+=bs
            ...
            for i in range(len(len1)):
                ...
                with open(f"./test_result/{name}.csv", 'a', encoding='utf-8-sig', newline='') as f:
                    ...
                    if valid[i]:
                        ...
                        #     flag = 1
                        writer.writerow(["," + src[2:-1], str(tgt), str(tgt), "", "", "预测正确"])
                        # print("#" * 50 + " 下面这些是完美预测公式：即预测公式和目标公式完全一模一样 " + "#" * 50)
                        # print('input: ' + src[2:-1])
                        # print('目标公式: ' + str(tgt))
                        # print('预测公式: ' + str(tgt))
                        # print('结果: ' + "预测正确\n")
            ...
            outputs = []
            for inp in inputs:
                outputs.append(check_hypothesis(inp, env.params.n_predictions))
