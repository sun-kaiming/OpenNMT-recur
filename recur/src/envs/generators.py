...
import copy
import os
# from train import obtain_parser
...
class RandomRecurrence(Generator):
    ...
    def evaluate(self, src, tgt, hyp, n_predictions=3):
        ...
        errors = []
        name = self.params.test_result_name
        ...
        with open(f"./test_result/{name}.csv",'a',encoding='utf-8-sig',newline='')as f:
            ...
            for i in range(n_predictions):
                try:
                    pred = hyp.val(src_hyp, deterministic=True)  #val函数可以使用hyp公式预测下面的term
                    src_hyp.extend(pred)
            ...
            for err in errors:
                if err!=0:
                    res="预测错误"
                # str(tgt), str(hyp),
            writer.writerow([","+' '.join( str(id) for id in src),str(tgt), str(hyp), ","+' '.join(str(id) for id in src_tgt[-n_predictions:]),","+' '.join(str(id) for id in src_hyp[-n_predictions:]),res])
            # print('input: ' + ' '.join( str(id) for id in src))
            # print('目标公式: ' + str(tgt))
            # print('预测公式: ' + str(hyp))
            # print('真实后10项: ' + ' '.join(str(id) for id in src_tgt[-n_predictions:]))
            # print('预测后10项: ' + ' '.join(str(id) for id in src_hyp[-n_predictions:]))
            #
            # print('结果: ' + res+"\n")
        return errors        
    ...
    def evaluate_without_target(self, src, hyp, n_predictions=3):
        ...
        n=n_predictions
        # print(src)
        if len(src)<n_predictions:
            n=int(len(src)/3)
        targets = src[-n:]
        src = src[:-n]
        name = "OEIS_"+self.params.test_result_name
        with open(f"./test_result/{name}.csv", 'a', encoding='utf-8-sig', newline='') as f:
            writer = csv.writer(f)

            # writer.writerow(["输入的整数数列" + '目标公式', "预测公式", "数列后10项", "预测后10项", "结果"])
            real_next_n=[]
            pre_next_n=[]
            for i in range(n_predictions):
                try:
                    pred = hyp.val(src)
                    pre_next_n.append(pred[0])
                    true = [targets[i]]
                    real_next_n.append(true[0])
                    src.extend(pred)
                    errors.append(max([abs(float(p-t)/float(t+1e-100)) for p,t in zip(pred, true)]))
                except:
                    return [-1 for _ in range(n_predictions)]
            ...
            writer.writerow([","+' '.join( str(id) for id in src),"", str(hyp), ","+' '.join(str(id) for id in real_next_n[-n_predictions:]),","+' '.join(str(id) for id in pre_next_n[-n_predictions:]),res])
            print('input: ' + ' '.join( str(id) for id in src))
            print('预测公式: ' + str(hyp))
            print(f'真实后{n}项: ' + ' '.join(str(id) for id in real_next_n[-n:]))
            print(f'预测后{n}项: ' + ' '.join(str(id) for id in pre_next_n[-n:]))
...
# if __name__ =='__main__': #测试单独获取数据，期望可以进行单步调试
#     params=obtain_parser()
#     env = build_env(params)
#
#
#     x, y, tree, infos = env.gen_expr(train=True,input_length_modulo=-1,nb_ops=None)
#
#     print(x)
#     print(y)
#     print(tree)
