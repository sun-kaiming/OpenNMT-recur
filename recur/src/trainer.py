...
class Trainer(object):
    def __init__(self, modules, env, params):
        ...
        if not params.eval_only:
            ...
            self.dataloader = {
                "recurrence": iter(self.env.create_train_iterator("recurrence", self.data_path, params))
                # for task in params.tasks
            }
            print("len(data_loader)===========:", len(self.env.create_train_iterator("recurrence", self.data_path, params)))
            # count=0
            # for i,data_i in enumerate(self.env.create_train_iterator(task, self.data_path, params)):
            #     count+=1
            # print("count",count)
            # exit()
    ...
    def set_new_train_iterator_params(self, args={}):
        ...
        if params.env_base_seed < 0:
            params.env_base_seed = np.random.randint(1_000_000_000)
        self.dataloader = {
            "recurrence": iter(self.env.create_train_iterator("recurrence", self.data_path, params, args))
            # for task in params.tasks
        }
    ...
    def reload_checkpoint(self):
        ...
        if self.params.fp16 and not self.params.nvidia_apex:
            ...
            print(self.scaler)
            if "scaler" in data:
                print("scaler in data")
            print("="*50)

            assert self.scler is None and "scaler" not in data
    ...
    def get_batch(self, task):
        """
        Return a training batch for a specific task. 返回特定任务的训练批次。
        """
        try:
            # self.dataloader = iter(self.dataloader[task])
            batch = next(self.dataloader[task])

            # for i,batch_i in enumerate(self.dataloader[task]) :
            #     print(i," batch ")
            # print("%%%%%%%%%%%%%%")
            # exit()
            # print("**00000")
    ...
    def export_data(self, task):
        """
        Export data to the disk. 将数据导出到磁盘。
        """
        env = self.env
    ...
    def obtatin_batchsize_data(self, task):
        (x1, len1), (x2, len2), trees, infos = self.get_batch(task)
        return (x1, len1), (x2, len2), trees, infos
