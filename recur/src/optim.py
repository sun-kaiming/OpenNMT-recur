...
class AdamInverseSqrtWithWarmup(Adam):
    ...
    根据更新数的反平方根衰减 LR。 我们还支持预热阶段，在该阶段我们线性增加学习率，
    从某个初始学习率 (`warmup-init-lr`) 直到配置的学习率 (`lr`)。
    此后，我们与更新次数成比例地衰减，衰减因子设置为与配置的学习率一致。
    During warmup:
        lrs = torch.linspace(warmup_init_lr, lr, warmup_updates)
        lr = lrs[update_num]
    After warmup:
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        self.decrease_num=2  #弥补btach_size太小，导致lr衰减过快的问题，按步骤平方进行衰减时;大于10000步时，batch_size=128时，
                                # 每过4个步骤学习率更新一次，就和batch_size=512时的学习率变化情况等价了
    ...
    """
    每四步更新一次学习率，消除batch_size=128的影响
    """
    # def get_lr_for_step(self, num_updates):
    #     if (num_updates-self.warmup_updates )%self.decrease_num==0 :
    #         if num_updates <= self.warmup_updates*self.decrease_num:
    #             num_updates=num_updates/self.decrease_num
    #             return self.warmup_init_lr + num_updates * self.lr_step
    #         else:
    #             # if num_updates>self.warmup_updates:
    #             num_updates=self.warmup_updates +(num_updates-self.warmup_updates*self.decrease_num )/self.decrease_num
    #             return self.decay_factor * (num_updates ** -self.exp_factor)
    #     else:
    #         return self.param_groups[0]['lr']

    """
    缩减相应大小的lr,经过实验，效果并不好
    """
    # def get_lr_for_step(self, num_updates):
    #     # if (num_updates-self.warmup_updates )%self.decrease_num==0:
    #     # if num_updates <= self.warmup_updates*self.decrease_num:
    ...
    #     else:
    #         # if num_updates>self.warmup_updates:
    #         # num_updates=self.warmup_updates*self.decrease_num +(num_updates-self.warmup_updates*self.decrease_num )/self.decrease_num
    #         return self.decay_factor * (num_updates ** -self.exp_factor)
    #     # else:
    #     #     return self.param_groups[0]['lr']
    #
    """
    先预热学习率4万步 到设置的初始学习率，然后使学习率保持不变
    """
    # def get_lr_for_step(self, num_updates):
    #     if (num_updates-self.warmup_updates )%self.decrease_num==0 :
    #         if num_updates <= self.warmup_updates*self.decrease_num:
    ...
    #
    """
    先预热学习率（每4步变化一次），然后衰减lr,这里减小衰减的幅度
    # """
    # def get_lr_for_step(self, num_updates):
    #     if (num_updates-self.warmup_updates )%self.decrease_num==0 :
    #         if num_updates <= self.warmup_updates*self.decrease_num:
    ...
    #             # if num_updates>self.warmup_updates:
    #             num_updates=self.warmup_updates*self.decrease_num +(num_updates-self.warmup_updates*self.decrease_num )/self.decrease_num
    #             return self.lr2*(self.warmup_updates*self.decrease_num)**self.exp_factor * (num_updates ** -self.exp_factor)
    #     else:
    #         return self.param_groups[0]['lr']
    ...
    def step(self, closure=None):
        ...
        for param_group in self.param_groups:
            param_group['num_updates'] += 1
            # if (param_group['num_updates'] - 10000) % self.decrease_num == 0:
            param_group['lr'] = self.get_lr_for_step(param_group['num_updates'])
...
class AdamCosineWithWarmup(Adam):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, warmup_updates=10000, warmup_init_lr=1e-7,
                 min_lr=1e-9, init_period=100000000, period_mult=1, lr_shrink=0.75, smooth=False):
        super().__init__(
            params,
            lr=warmup_init_lr,
            betas=betas,
